...
class IndexController extends Controller
{
    ...
    public function index(){
        ...
        $item = DB::table('review')
            ->leftJoin('user','user.id','review.uid')
            ->where('aid',$id)->get(['user.username','review.*']);

        return view('index.index',['info'=>$res,'item' => $item]);
    }
     ...
     */
    public function map(){
        $res = DB::table('article')->orderBy('hash','desc')->get(['author','id']);
        return view('index.map',['item'=>$res]);
    }
    ...
    public function info($id){
        ...
        if(!$res){
            return response('<div style="text-align: center;padding: 50px">逗逼，没有这条句子，换一条吧！</div><div style="text-align: center;"><a href="/map.html">返回</a></div>',404);
        }
        return view('index.info',['info'=>$res,'item' => $item]);
    }
    ...
    public function like(Request $request){
        ...
        $res = DB::table('article')->where('id',$id)->increment('like');
        if($res){
            return [
                'code' => 200,
                'msg'  =>'Success.',
    ...
    public function review(Request $request){
        ...
        // 插入用户
        $id = $this->createUser(['username' => $username]);
        if(!$id){
            return [
                'code' => 500,
                'msg'  =>'用户评论异常',
        ...
        $res = DB::table('article')->where('id',$aid)->increment('review');
    ...
    public function reviewLike(Request $request){
        ...
        $res = DB::table('review')->where('id',$id)->increment('like');
        if($res){
            return [
                'code' => 200,
                'msg'  =>'Success.',
    ...
    public function reply(Request $request){
        ...
        $res = DB::table('article')->where('id',$aid)->increment('review');
        $res = DB::table('review')->where('id',$aid)->increment('review');
    ...
    public function createUser($data = []){
        ...
        $id = DB::table('user')->insertGetId(
            ['username' => $data['username'],'email'=>'','phone'=>'','create_time'=>date('Y-m-d H:i:s')]
        );
    ...
    public function randUsername(){
        ...
        $tou_num=rand(0,331);
        $wei_num=rand(0,325);
        return $tou[$tou_num].$wei[$wei_num];
    }
}
